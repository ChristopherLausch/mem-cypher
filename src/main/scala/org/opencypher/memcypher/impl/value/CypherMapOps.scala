...
import org.opencypher.memcypher.impl.table.RecordHeaderUtils.RichExpression
...
import org.opencypher.okapi.api.value.CypherValue.{CypherBoolean, CypherInteger, CypherMap, CypherString, CypherValue}
...
object CypherMapOps {
  ...
  implicit class RichCypherMap(map: CypherMap) {
    ...
    def evaluate(expr: Expr)(implicit header: RecordHeader, context: MemRuntimeContext): CypherValue = {
      ...
      expr match {
        ...
        case HasType(rel, relType) =>
          //Old Version used CTWildcard as default value in the constructor -> replaced with CTAny?
          //evaluate(Type(rel)()) == CypherString(relType.name)
          evaluate(Type(rel)(CTAny)) == CypherString(relType.name)

        case Labels(innerExpr) =>
          logger.info(s"Labels: `$innerExpr` in: $map")
          val node = Var(innerExpr.columnName)(CTNode)
          //val labelExprs = header.label(node)
        ...
        case GreaterThan(lhs, rhs) =>
          ...
          //ToDo Check why TrueLit and FalseLit are not found, they exist as objects in Expr.scala
        case _: TrueLit =>
          true
    ...
    def nest(header: RecordHeader): CypherMap = {
      ...
      val values = header.vars.map { field =>
